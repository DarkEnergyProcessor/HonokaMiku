...
inline const char* CompilerName()
{
	...
	sprintf(name,"%s",__VERSION__);
#elif defined(__GNUC__) || defined(__GNUG__)
	/* GCC. -----------------------------------------------------*/
	sprintf(name,"gcc-%s",__VERSION__);
	...
	/* IBM XL C/C++. -------------------------------------------- */
	sprintf(name,"IBM XL C/C++ Version %s",__xlc__);
...
#endif
	sprintf(name,"Oracle Solaris Studio Version %x.%x.%x",v/0x1000,(v/0x10)%0x100,v%0x10);
#else
